...
class Transition():
	...
	def __call__(self,currentStates,currentActions):
		...
		newVelocities = [renormalVector(np.add(currentVelocities[i],np.divide(currentActions[i],2.0)),self.speedList[i]) for i in range(numberObjects)]
		newPositions = [np.add(currentPositions[i], newVelocities[i]) for i in range(numberObjects)]
...
if __name__=="__main__":
	...
	currentStates=pd.DataFrame(statesList,index=[0,1,2],columns=['positionX','positionY','velocityX','velocityY'])
	speedList=[5,3,3]
