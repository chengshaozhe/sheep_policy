...
class SampleTrajectory():
    ...
    def __call__(self, policy):
        ...
        for t in range(self.maxTimeStep):
            ...
            trajectory.append((pool.apply_async(self.transitionFunction,(oldState, action)),action))
        ...
        print((time.time() - start_time)/len(trajectory))
        return trajectory
...
class PolicyGradient():
    ...
    def __call__(self, model, approximatePolicy, sampleTrajectory, accumulateRewards, train, saver):
        for episodeIndex in range(self.maxEpisode):
            ...
            # 	pool.map(sampleTrajectory, policy)
            # 	# episode = [pool.map(sampleTrajectory, policy) for index in range(self.numTrajectory)]
            normalizedAccumulatedRewardsEpisode = [
