...
class ActorNetwork:
    ...
    def create_actor_network(self, state_size, action_size):
        ...
        out = Dense(action_size, activation='tanh')(h1)
        out = Lambda(lambda x: x * self.action_bound)(out)        
...
if __name__ == '__main__':
    ...
    action_size = 2
    action_bound = speedList[sheepIdentity]
    ...
    actor = ActorNetwork(sess, state_size, action_size, action_bound, BATCH_SIZE, TAU, LEARNING_RATE_ACTOR)
    ...
    # Test_wight_name = 3000
    # actor.model.load_weights('./save/Continuous_SingleWolf_DDPG_episode_' + str(Test_wight_name) + "-actormodel.h5")
    # critic.model.load_weights('./save/Continuous_SingleWolf_DDPG_episode_' + str(Test_wight_name) + "-criticmodel.h5")
    # actor.target_model.load_weights('./save/Continuous_SingleWolf_DDPG_episode_' + str(Test_wight_name) + "-actormodel.h5")
    # critic.target_model.load_weights('./save/Continuous_SingleWolf_DDPG_episode_' + str(Test_wight_name) + "-criticmodel.h5")
    ...
    for e in range(num_opisodes):
        ...
        for time in range(1000):
            ...
            if np.random.rand() <= epsilon:
                ...
                action_input[0] = np.random.uniform(-action_bound,action_bound, action_size)
            else:
                action_input[0] = actor(oldStates_input)

        #Ornstein-Uhlenbeck Process
            # noise_t = np.zeros([1,action_size])
            # a_t_original =  actor.model.predict(oldStates_input)
            # noise_t[0][0] = max(epsilon, 0) * OU(a_t_original[0][0],  0.0 , 0.60, 0.30)
            # noise_t[0][1] = max(epsilon, 0) * OU(a_t_original[0][1],  0.0 , 0.60, 0.30)
            # action_input[0][0] = a_t_original[0][0] + noise_t[0][0]
            # action_input[0][1] = a_t_original[0][1] + noise_t[0][1]



            sheepAction = action_input[0]
        ...
        if np.mod(e, 10) == 0:
            ...
            data_path = os.path.join(module_path, "saveDDPG")
            name = 'Continuous_SingleWolf_DDPG_episode_' + str(e) + '-'
        ...
        if TEST:
            if e % 100 == 0:
                ...
                for wolfPrecision in assumeWolfPrecisionList:
                    ...
                    for test_episode in range(test_episodes):
                        ...
                        for time in range(1000):
                            ...
                            oldStates_input = np.reshape(oldStates_array,[1,state_size])
                            action_input = actor(oldStates_input)

                            sheepAction = action_input
                ...
                filename = 'DDPG_continuous_test_episode' + '-'  + str(e) + 'test' + str(test_episodes)
